...
Congratulations! Somehow, you got your hands on one of BeagleBurner boards. Or
maybe you just curious how one is used.
...
## Burning sd card content

To burn some content from one of the slots to an SD Card do like so:

1. Power on the BeagleBurner board.
2. Slot LEDs will glow for a short time, and will be off when the board is
   ready to go.
3. Insert an SD card.
4. Slot LEDs will start to blink -- now you can press one of the slot buttons.
5. Wait until "DONE" LED starts to glow.
6. Take off the SD card.

You can burn as many SD cards you wish without powering off the board.

If "FAIL" LED started to glow, reboot the board and try again. If it still
doesn't work, check that you have files for the selected slot. If it doesn't
work even if the files are okay, check your SD card.


## Updating files

To change the files used for flashing sd cards:

1. Disconnect the BeagleBurner board from power.
2. Press and hold one of the slot buttons.
3. Connect it to a PC.
4. Copy the files you need.
5. Do "Eject USB Mass Storage Device" before disconnecting the device.
